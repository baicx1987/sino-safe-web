...
const tableId = 'ssuId';
const tableDelete = 'ssuIsDeleted';

@connect(state => ({
  skinUse: state.skinUse,
}))
@Form.create()
export default class SkinUse extends PureComponent {
  state = {
  ...
  renderSimpleQueryForm() {
    const { skinUse: { unitSelectData } } = this.props;
    const unitOptions = [];
    if (unitSelectData) {
  ...
  render() {
    const { skinUse: { loading: skinUseLoading, data, viewData } } = this.props;
    ...
    const columns = [
      {
        title: '考试计划',
        dataIndex: 'ssuPlanName',
        sorter: true,
      },
      {
        title: '行政区',
        dataIndex: 'ssuAreaName',
      },
      {
        title: '考试皮肤',
        dataIndex: 'ssuSkinName',
      },
      {
        title: '状态',
        dataIndex: 'ssuIsDeleted',
        sorter: true,
        render(val) {
          return (<Badge
            status={
    ...
    const detailColumns = [
      {
        title: '创建时间',
        dataIndex: 'ssuGmtCreate',
        sorter: true,
        render: val => <span>{moment(val).format('YYYY-MM-DD HH:mm:ss')}</span>,
      },
      {
        title: '最后操作时间',
        dataIndex: 'ssuGmtModified',
        sorter: true,
        render: val => <span>{moment(val).format('YYYY-MM-DD HH:mm:ss')}</span>,
      },
    ];
